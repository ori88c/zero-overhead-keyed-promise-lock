 ...
 */
class ZeroOverheadKeyedLock {
    constructor() {
        this._keyToLock = new Map();
    }
    /**
     ...
     */
    async executeExclusive(key, criticalTask) {
        const lock = this._getOrCreateLock(key);
        return lock.executeExclusive(async () => {
            try {
    ...
    _getOrCreateLock(key) {
        this._validateKey(key);
        let lock = this._keyToLock.get(key);
        if (!lock) {
            lock = new zero_overhead_promise_lock_1.ZeroOverheadLock();
            this._keyToLock.set(key, lock);
        }
        return lock;
    }
    _getLockIfExists(key) {
        this._validateKey(key);
        return this._keyToLock.get(key);
    }
}
exports.ZeroOverheadKeyedLock = ZeroOverheadKeyedLock;
//# sourceMappingURL=zero-overhead-keyed-promise-lock.js.map